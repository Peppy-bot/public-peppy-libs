...
/// Stable, non-secret build marker for release artifact verification. Peppy's
/// release gate inspects the managed `zenohd` binary for this exact value so a
/// stock Zenoh 1.9 artifact cannot be substituted through packaging or PATH.
pub static PEPPY_TLS_TRUST_POLICY_MARKER: &str =
...
impl<'a> TlsClientConfig<'a> {
    pub async fn new(config: &'a Config<'_>) -> ZResult<Self> {
        ...
        // A configured private CA is a trust boundary, not an addition to the
        // machine's public roots. Mixing the two lets any publicly trusted CA
        // authenticate a managed federation endpoint. With no explicit bundle,
        // delegate to rustls-platform-verifier so the operating system remains
        // the source of truth instead of shipping a separate static root set.
        let custom_root_cert_store = load_trust_anchors(config)?;
        tracing::debug!(
            trust_policy = PEPPY_TLS_TRUST_POLICY_MARKER,
            "Applying Peppy TLS client trust policy"
        );
        ...
        // Keep mutual TLS on TLS 1.3 only. Plain server-authenticated TLS keeps
        // rustls's safe defaults for compatibility with non-federation links.
        let builder = if tls_client_server_auth {
            ClientConfig::builder_with_protocol_versions(&[&TLS13])
        } else {
            ClientConfig::builder()
        };

        let builder = match custom_root_cert_store {
            Some(root_cert_store) => {
                tracing::debug!("Loading exclusive user-configured trust anchors.");
                if tls_server_name_verification {
                    ...
                    builder
                        ...
                        .with_custom_certificate_verifier(Arc::new(
                            WebPkiVerifierAnyServerName::new(root_cert_store),
                        ))
                }
            }
            None => {
                tracing::debug!("Loading the operating system TLS trust policy.");
                if !tls_server_name_verification {
                    // Platform verifiers combine chain and host verification;
                    // there is no sound cross-platform way to retain OS trust
                    // while selectively disabling only the name check. Prefer
                    // the secure platform behavior over silently falling back
                    // to a different root source.
                    tracing::warn!(
                        "Ignoring verify_name_on_connect=false because no custom CA is configured; the platform verifier always checks the server name"
                    );
                }
                builder
                    .with_platform_verifier()
                    .map_err(|e| zerror!("Failed to configure platform TLS verifier: {e}"))?
            }
        };
...
fn process_pem(pem: &mut dyn io::BufRead) -> ZResult<Vec<TrustAnchor<'static>>> {
    ...
    if trust_anchors.is_empty() {
        bail!("Configured TLS root CA bundle contained no certificates.");
    }
