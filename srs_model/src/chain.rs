//! The URDF link/joint tree, and forward kinematics over a path through it.
//!
...
impl Chain {
    ...
    pub(crate) fn from_robot(robot: &urdf_rs::Robot) -> Result<Self, String> {
    ...
    pub(crate) fn link_index(&self, name: &str) -> Option<usize> {
        self.by_name.get(name).copied()
    }

    pub(crate) fn link(&self, i: usize) -> &Link {
        &self.links[i]
    }

    pub(crate) fn joint(&self, i: usize) -> &Joint {
        &self.joints[i]
    }

    pub(crate) fn children_of(&self, link: usize) -> &[usize] {
        &self.children[link]
    }

    /// Joint indices from the root down to `link`, in order. Empty for the root.
    pub(crate) fn path_to(&self, link: usize) -> Vec<usize> {
    ...
    pub(crate) fn fixed_path_to(&self, from: usize, to: &str) -> Option<Isometry3<f64>> {
    ...
    pub(crate) fn subtree_has_revolute(&self, link: usize) -> bool {
