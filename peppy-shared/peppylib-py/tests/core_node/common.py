...
async def start_router_and_runner(tmp_path: Path):
    ...
    # The node runner opens its session under a workspace namespace; the
    # standalone config carries none, so it resolves to the `local` namespace.
    # The stub server must open under that same namespace — zenoh sessions
    # only interoperate when their namespaces match — or the runner's service
    # queries and topic subscriptions never reach the stub. Passing no
    # namespace resolves to `local`, matching the runner.
