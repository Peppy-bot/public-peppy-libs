...
/// Marks an action whose goal may belong to a federated launch, implemented by
/// the `methods!` expansion for every entry declared `scope: launch`.
///
/// Which actions are launch-scoped is a property of the protocol, not of one
...
/// scope at all.
///
/// Declaring `scope: launch` on an action whose goal codec has no
...
/// Emits the [`LaunchScoped`] impl for an action entry declared
/// `scope: launch` — or nothing, when the entry declares no scope. Unlike
