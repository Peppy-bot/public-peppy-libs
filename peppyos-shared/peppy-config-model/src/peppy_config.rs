...
//! template and its comment-preserving completion) lives in the peppyos
//! `daemon-config` crate: only the daemon reads or writes that file. What
//! stays here is the slice consumed beyond the daemon: [`PeerConfig`] (pmi
//! builds its subscriber buffer sizes from it), the buffer and grace
//! defaults `runtime.rs` uses for runtime-config serde fallbacks, and the
//! shutdown timing contract ([`EVENT_LOOP_JOIN_BUDGET_SECS`] read by
//! `peppylib-py`, plus [`RUNTIME_FINALIZE_MARGIN_SECS`] which the daemon
//! stacks on top of it when sizing force-kill deadlines).
