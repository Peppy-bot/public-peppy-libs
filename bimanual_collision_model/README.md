...
// error is a plain String rather than a CollisionError. The last argument is
// the fit tolerance in metres: how far the hull may stand off the cloud.
let wall = Obstacle::fit("wall", &points, 0.001)?;
model.add_obstacle(wall)?;
```

The tolerance is the caller's cost lever. The fit adds supporting planes until
the hull is within it, and every plane is per-tick work once the obstacle is
live, so a tighter fit is a permanently dearer one. A flat-faced body pays
nothing for tightness (a wall hulls to its eight corners at any tolerance), but
a curved one needs roughly a hundredth of its radius: a 20 cm ball wants 2 mm, a
1 m ball 10 mm. Asking for tighter than the shape allows is refused by name
rather than silently charged to the control loop, and a looser fit is always the
conservative direction, since the hull contains the cloud either way and only
grows.
